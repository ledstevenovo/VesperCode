...
v1 Agent 主循环采用顺序单轮模型：同一运行任一时刻最多存在一个活动 `AgentTurn`。每个逻辑 turn 恰好关联一次 LLM 调用 attempt；该 attempt 至多接受一个权威调用结果，该 turn 恰好形成一个完成结果。3.1 的 `SAME_ATTEMPT_REPLAY` 只允许相同 attempt 标识与相同规范输入返回首次已经形成的同一权威调用结果，绝不再次调用 LLM 适配器，也不创建新 turn。尚无权威调用结果时不得重发：已经开始的调用按既有调用失败路线关闭，尚未开始的调用按既有启动前取消或内部中止路线关闭；不得新增 replay 状态。迟到结果不得改写后续 turn、候选或阶段。
...
`SAME_ATTEMPT_REPLAY` 只有在同一 LLM call attempt 的首次权威结果已经形成时，才可把该同一结果连同原 `AgentFeedbackConsumptionManifest`、完整 reservations 和 `ContextProjection` 返回给相同标识与相同规范输入；它绝不得再次调用适配器、重新选择反馈、创建第二组 reservation 或创建新 turn。尚无权威结果时不得以该处置重发，必须按 3.5.1 的既有调用失败或启动前终止路线关闭，且不得新增 replay 状态。turn 已有权威终态后到达的迟到响应或其他结果不得重新打开 turn、Manifest 或 reservations，也不得形成第二个 Accepted、Rejected 或失败结果。
...
1. 同一运行任一时刻最多存在一个活动 `AgentTurn`；迟到或陈旧的终态提交必须被拒绝，且不得形成第二个权威结果。
2. 当前 phase 必须与 `AgentTurnSubject` 精确匹配；`ReproductionTurnSubject` 与 `RepairTurnSubject` 跨阶段使用时必须被拒绝。
3. mandatory context 超过硬预算时不得调用真实或 Mock LLM，并且必须形成既有的确定性失败或停止结果。
4. 真实 LLM 请求的精确披露内容必须与 3.5.3 引用的既有有效授权合同 `DisclosureGrant` 精确匹配；任一项不匹配都不得发送。
...
14. 每个选入反馈必须恰好对应一个 `FeedbackConsumptionReservationRecord`。
15. `AcceptedTurnOutputRecord` 必须全量消费该 `AgentFeedbackConsumptionManifest` 的 reservations。
16. `RejectedTurnOutputRecord` 必须全量消费该 `AgentFeedbackConsumptionManifest` 的 reservations。
17. 终态调用失败、输出前取消和内部中止必须全量关闭 reservations 且不消费。
18. `SAME_ATTEMPT_REPLAY` 只能为相同标识与相同规范输入返回首次已经形成的同一权威结果，并复用相同 Manifest、reservations 和 `ContextProjection`；不得重新调用外部适配器、重新选择反馈或形成第二个结果。
19. 尚无权威结果时不得以 `SAME_ATTEMPT_REPLAY` 重发；必须走既有准入 `UNKNOWN | NOT_RUN(CONTROL_PLANE_ABORTED)` 或 LLM 调用失败／启动前终止路线，不得新增 replay 状态，且迟到结果不得重新打开既有 turn、Manifest 或 reservations。
20. 响应后的不可恢复处理故障必须停止为 `STOPPED(INTERNAL_ERROR)`，全量关闭 reservations 且不消费，并且不得重新请求供应商。
