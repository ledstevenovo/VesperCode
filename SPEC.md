...
1. 同一运行任一时刻最多存在一个活动 `AgentTurn`；迟到或陈旧 turn 的领取请求或结果提交必须被拒绝，且不得形成第二个权威结果。
...
8. 路径逃逸、带有 Shell 或正则语义的输入，以及越界 glob、查询、页、行或字节输入都必须被拒绝。
...
18. `SAME_ATTEMPT_REPLAY` 必须复用相同 Manifest 和 reservations，不得重新选择反馈。
19. 迟到响应或传输结果不得重新打开 turn、Manifest 或 reservations，也不得形成第二个结果。
20. 响应后的不可恢复处理故障必须停止为 `STOPPED(INTERNAL_ERROR)`，全量关闭 reservations 且不消费，并且不得重新请求供应商。
21. Task 4—6 删除的旧类型不得存在悬空规范引用；只有明确的非目标说明或历史说明可以提及相应概念。
