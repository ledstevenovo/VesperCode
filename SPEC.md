...
> 版本：SPEC v3  
> 状态：SPEC v3 已获正式实现授权（`FORMAL_READY`，2026-08-03）。§11.2 的冷启动前文档检查、隔离且可丢弃的陌生 Agent 试作及其反馈记录均已完成；CS-03 已达到预先声明的 T01.1 边界，且没有未解决的 `BLOCKING` 或 `CLARIFY` finding。过程证据见 `SPEC_PROCESS.md` 与 `AGENT_LOG.md`。本授权仅允许按 `PLAN.md` 开始正式任务；它不把冷启动产物当作实现证据，也不表示任何实现任务、交付或发布已完成。
...
跨会话消费者只能依赖上述可观察语义和被引用的规范字段，不得依赖某个 PLAN task 曾列出的 Python 类名、helper 名、参数顺序或模块内组织。只在单一实现会话内使用的私有函数、局部类和测试 fixture 不是 SPEC 契约，由实现者在不改变可观察行为、身份、不变量、错误和验收条件的前提下决定。若两个实现会话需要新增共享字段、序列化形式、状态转换、错误码或身份输入，必须先修改 SPEC，并在 `SPEC_PROCESS.md` 中记录变更原因；若该变更影响已选冷启动 task 的行为、接口、RED、验证步骤或完成条件，则必须重新执行该 task 的冷启动试作；不能只在 PLAN 或实现中约定。
