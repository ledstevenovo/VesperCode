...
下表是运行生命周期转换的唯一规范来源。
这里的“唯一”按触发事件归属：3.2.7 是表中所列主流程、等待／批准以及正式／Demo 启动关闭事件的唯一规范来源；3.2.8 的取消／跨阶段终止、3.2.9 与 3.10 的持久化恢复各自仍是对应事件的权威，本表不复制这些合同。
...
3.10 只负责正式验证结果；3.5 创建下一 `AgentTurn` 时，从该 `FormalValidationResult` 确定性生成有界结构化反馈。

上述正式重启行只引用 3.3.7 的对象级闭合细节，不复制其算法；3.3.7 仍是对象级闭合权威。`RUNNING(PERSISTENCE)` 和 `RECOVERY_REQUIRED` 明确排除在该正式重启行之外，只能走 3.2.9 与 3.10 的持久化恢复路线。
