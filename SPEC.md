...
- 当前进程仍持有 OS 排他锁，且提交绑定该运行创建时的 `workspace_lease_ref`（按 3.3）；
- 当前 Agent 来源绑定、binding revision 与精确来源树摘要；
- 不存在活动轮次、优先取消或阻断事项；
- 适用的披露授权已经满足 3.9。
...
### 3.5.10 不可变 Agent 来源发布与当前绑定

每个允许文件检查的轮次主体必须对应一个不可变 `AgentWorkspacePublication`。该名称在 v1 只表示 Agent 可见来源树的逻辑发布，不表示或持有物理 `ExecutionWorkspace`；它只能发布当前 `SnapshotTree` 或 `CandidateTree` 的引用与摘要，并精确绑定轮次主体、Manifest 和 phase-entry。文件工具不持有长期物理执行副本。

~~~text
AgentWorkspacePublication {
  schema_version
  agent_source_publication_ref
  owner_run_id
  phase_entry_ref
  turn_subject_ref_and_digest
  manifest_ref_and_digest
  source_tree_ref_and_digest
  publication_digest
}

CurrentAgentSourceBinding {
  schema_version
  owner_run_id
  phase_entry_ref
  binding_revision
  agent_source_publication_ref_and_digest
  bound_turn_subject_ref_and_digest
  manifest_ref_and_digest
  source_tree_ref_and_digest
  current_candidate_revision_ref_and_digest?
...
`REPRODUCTION` 的来源只能是当前 `ReproductionTurnSubject` 绑定的 `SnapshotTree`；`AGENT_LOOP` 的来源只能是当前 `RepairTurnSubject` 绑定的 `CandidateTree`。一个 publication 只能对应一个不可变主体、来源树和 Manifest。publication 在成为当前 `CurrentAgentSourceBinding` 前不得被文件工具消费；当前绑定通过不可变 binding revision 链和 CAS 前移，不得原地改写。

文件工具开始前必须同时验证当前来源绑定、binding revision、phase-entry、主体、Manifest 和精确来源树引用与摘要。每个文件工具 attempt 都独立消费该逻辑来源绑定；长期物理目录、领取状态或活动工具所有权均不构成权威状态。

`ApplyCandidatePatchAction` 在动作和策略层仍绑定当前 Agent 来源绑定，以证明模型检查和提案所针对的精确候选视图；PatchEngine 实际上只能把补丁请求交给 3.6，并从该绑定指向的 `CandidateTree` 创建 staging tree、新 `CandidateRevision` 和新 `CandidateTree`。它不得写入或重新发布旧来源树。

补丁成功必须采用以下原子激活协议：

~~~text
3.6 生成新 CandidateRevision 和 CandidateTree
→ 为新 CandidateTree 发布不可变 AgentWorkspacePublication
→ CAS 重验旧候选、旧 CurrentAgentSourceBinding 和 phase-entry 仍为当前
→ 原子激活新候选、新 CurrentAgentSourceBinding 和补丁成功结果
~~~

最终 CAS 必须把新候选激活、新 binding revision 和补丁成功结果作为同一控制面结果提交。新来源发布或最终 CAS 任一失败时，旧候选和旧来源绑定保持当前且逐字节不变；新修订不得成为当前修订，补丁不得报告成功。已经生成但未激活的新修订只能作为未激活历史工件由 3.6 的合同处置，其来源发布始终不得成为当前绑定。
...
动作中的 `requested_path`、`root_path`、`path_glob`、深度和体量均是不可信请求值。所有路径必须采用仓库根相对的规范 `/` 分隔形式；`root_path = ""` 是表示仓库根的唯一例外，其他路径必须非空。拒绝绝对路径、盘符或 UNC 路径、空段、`.`、`..`、ADS、NUL、平台保留名称、反斜杠以及规范化后与原输入不同的形式。`root_path` 必须是当前 subject tree 中的现有目录，文件读取路径必须是其中的现有普通文件；符号链接及其他特殊条目的处理只能由 3.4 冻结的文件系统画像决定，模型不能借其逃逸 subject tree。

`path_glob` 使用版本化受限 glob 语法，由 Harness 自身解释，不得交给 Shell 或宿主 glob。v1 不支持 brace expansion、正则、命令替换、环境变量、宿主转义或平台相关匹配语义。请求超过深度、页大小、glob、读取或查询硬上限时必须返回稳定输入错误，不得静默裁剪、缩小范围或替换为默认值。规范请求摘要必须绑定工具画像、当前来源绑定、规范查询和全部有效上限。
...
AgentFileToolAttemptStartRecord {
  ...
  action_kind
  current_source_binding_ref_and_digest
  expected_source_binding_revision
  source_tree_ref_and_digest
...
AgentFileToolAttemptOutcome =
  AgentFileToolPublishedRecord {
    ...
    execution_workspace_evidence_ref_and_digest
    execution_workspace_cleanup_result_ref_and_digest
  }
...
相同 `tool_attempt_id` 与相同规范输入重放时必须返回首次权威终态，不重新枚举或读取文件；相同 ID 与不同规范输入必须返回 `TOOL_ATTEMPT_ID_REUSE_CONFLICT`。当前来源绑定、subject、来源树或 binding revision 已变化时，迟到结果不得发布。临时正文已经释放时，重放原 attempt 仍返回原持久元数据以及正文不可用状态，不得伪造或重读正文；需要再次取得正文时，控制面必须为语义相同的请求创建新的 attempt，并且不能声称原结果正文仍然存在。
...
cursor 状态只能是 `ACTIVE` 或 `INVALIDATED`，并绑定同一运行、动作种类、来源绑定 revision、精确来源树、规范查询和分页画像。它可以跨连续 Agent turn 使用，但不能跨候选修订、来源绑定前移或运行迁移。binding 前移、来源树或 subject 变化时返回 `STALE_CONTINUATION_CURSOR`；格式、认证、摘要、所有者或动作类型错误时返回 `CONTINUATION_CURSOR_INVALID`，且必须在文件枚举或正文读取前拒绝。

相同活动 cursor 的重放始终指向同一 `next_result_ordinal` 和同一逻辑页面，不得消费 cursor、重复推进或跳页。每个继续动作仍创建独立文件工具 attempt；只有当前来源绑定仍精确匹配，并且该 attempt 的一次性执行副本通过 3.4 完整性验证后，才可以重新形成该页的权威结果。最后一页不签发新 cursor；控制面失效 cursor 只能追加失效事实，不得把已有 token 改写成另一查询的能力。
...
### 3.5.19 来源绑定与一次性执行完整性门

每次列表、读取和搜索都必须消费精确的当前 `CurrentAgentSourceBinding`，而不能把已物化或已清理的物理目录当作持续权威状态。开始访问前至少验证：binding revision、phase-entry、subject、Manifest 均仍为当前；绑定中的 `SnapshotTree` 或 `CandidateTree` 引用与摘要精确匹配；请求条目存在于该来源树。每个 attempt 随后按 3.4 从这棵精确来源树创建独立全新的一次性执行副本，满足硬容量限制并完成 pre-run 验证，再在文件工具 consumer 启动前形成该副本的不可变 `ExecutionWorkspaceEvidence`；只有该 evidence 已完整绑定精确来源、consumer 和副本身份时才可以启动 consumer。不同 attempt 不共享物理副本、缓存或运行期产物。

列表、读取或搜索完成后，系统必须按 3.4 完成 post-run 验证。需要保留的正文必须先转存到执行根之外的 `TransientToolContentBuffer` 并校验精确摘要；随后引用 consumer 启动前已形成的 `ExecutionWorkspaceEvidence` 执行清理，并只形成该 attempt 的 `ExecutionWorkspaceCleanupResult`，清理为 `QUARANTINED` 时还必须存在对应的 `ExecutionWorkspaceQuarantineRecord`。只有既有 evidence 与 cleanup result 绑定完整且清理结果允许发布时，才可以原子发布工具结果及适用的临时正文引用；已清理或隔离的物理目录不得成为后续工具、反馈或上下文的权威来源。

来源绑定、pre-run、post-run、转存或证据验证任一失败时，必须返回 `TOOL_RESULT_INTEGRITY_INVALID`，不得发布成功 payload、临时正文或可进入 `ContextProjection` 的结果，并仍须按 3.4 尝试安全清理。`BOUNDARY_UNSAFE` 不得发布工具结果。控制面不得改写源 `SnapshotTree` 或 `CandidateTree`，也不得在本节引入长期资源所有权、响应恢复或通用故障处置路线。`TOOL_RESULT_INTEGRITY_INVALID` 与 `TRANSIENT_CONTENT_UNAVAILABLE` 不得合并：前者表示本次读取不能证明与精确来源一致，后者表示曾经形成的有效负载已按临时正文生命周期清理。

第二部分至少满足以下可确定性验证点：

1. 给定当前 Agent 来源绑定，任一文件工具和 `ApplyCandidatePatchAction` 均不得改变其来源树根摘要或任一文件字节。
2. 给定新候选已生成但新来源发布或最终 CAS 失效，当前候选与旧来源绑定必须保持有效，且补丁不得报告成功。
3. 给定新来源 publication 已形成但尚未成为当前 binding，文件工具必须拒绝消费。
4. 给定相同 tool attempt ID 与相同规范输入，重放必须返回首次终态且不重新读取；相同 ID 与不同输入必须返回冲突。
5. 给定请求路径、glob、深度、页大小或体量超过绑定画像，系统必须返回稳定输入错误，不得静默收紧后执行。
6. 给定被修改、跨来源绑定或跨候选使用的 cursor，系统必须在枚举或读取前拒绝；给定同一有效 cursor 重放，逻辑页面不得变化或推进。
...
10. 给定一次性执行副本与精确来源树摘要不一致，或工具结果缺少匹配的 `ExecutionWorkspaceEvidence` 与 cleanup result，系统不得返回或披露可疑正文，不得把它加入任何 `ContextProjection`。
...
AgentFeedbackTargetScopeBinding {
  ...
  target_agent_source_binding_ref_and_digest?
  bound_target_scope_revision
  binding_digest
}
~~~

该绑定必须复用 3.5.2 的 `AgentTurnSubject`，发布后不可修改。`bound_target_scope_revision` 表示创建绑定时观察到的目标作用域版本，不表示对象可以原地升级。phase-entry、subject、Manifest、候选或适用的当前来源绑定变化时必须创建新作用域对象；旧反馈只保留为历史事实。
...
反馈不得自动迁移到任何不同的 phase-entry、Manifest、候选修订、当前来源绑定或目标作用域，即使候选之间存在祖先—后代关系。补丁成功反馈只能绑定已经原子激活的新候选修订和新 `CurrentAgentSourceBinding`；补丁失败绑定原候选；正式验证可反馈失败绑定返回后的新 `AGENT_LOOP` phase-entry；用户修订绑定等待关闭后重新进入的来源阶段实例。不得先发布反馈，再补写其目标作用域。

当前来源绑定的存在由资格画像针对具体来源和反馈类型确定：
...
`CandidatePatchFeedback` 成功时必须为 `REQUIRED` 并指向新激活来源绑定；反馈语义仅依赖 Manifest、候选树或结构化权威结果时应为 `FORBIDDEN`；需要当前来源专属消费能力或当前临时正文时必须为 `REQUIRED`。模型、用户、调用方和单次发布逻辑均不能选择是否填写该字段。

`SourceBindingRequirement` 只决定反馈目标是否必须匹配当前来源视图，不表示产生某个既有工具结果的物理副本仍存在。`FILE_TOOL_RESULT` 的权威来源必须引用 3.5.12 已发布记录及其不可变 `ExecutionWorkspaceEvidence`、`ExecutionWorkspaceCleanupResult`；这组历史证据证明结果如何产生，但不能替代 `target_agent_source_binding_ref_and_digest`，也不能授权后续文件访问。
...
文件观察反馈的摘要必须包含精确 payload digest、规范路径、范围、创建反馈时观察到的可用状态，以及产生该结果的一次性 `ExecutionWorkspaceEvidence` 和 cleanup result 摘要；它不得包含已清理物理目录的位置，也不得把历史执行证据解释为当前来源绑定。诊断投影包含排序、裁剪和脱敏后的语义摘要，不包含物理日志地址。临时正文后续释放不修改原 `feedback_digest`，只影响消费时能否取得 detail。反馈发布、当前可消费和已经发送是三个不同事实：
...
+ 新 AgentWorkspacePublication 及 CurrentAgentSourceBinding 前移
+ 候选补丁权威结果
+ FeedbackEligibilityRecord
...
新来源绑定未成功激活时不得发布指向新候选的反馈。稳定的 `INELIGIBLE` 不得回滚已经满足 3.6 和本节切换合同的领域事实。
...
跨后代候选延续必须由版本化 `FeedbackContinuationProfile` 允许，并创建显式 continuation。continuation 必须引用候选 ancestry proof，重新验证来源、原目标绑定、新目标绑定、当前来源绑定和临时正文要求，不能成环，不能生成第二条反馈，也不能修改原 feedback 或 target scope。其状态空间为：
...
每次候选转换允许延续的最大数量、允许的来源和 slot 类型、当前来源绑定／临时正文要求及严格选择顺序均由 `FeedbackContinuationProfile` 固定。超过上限时，控制面按规范顺序选择前 N 条，并在候选和来源绑定激活事务中原子发布：
...
- 候选、Manifest 和要求存在的当前 Agent 来源绑定仍有效；
- 来源为文件工具结果时，产生该结果的一次性 `ExecutionWorkspaceEvidence` 与 cleanup result 绑定完整且摘要一致；
- 没有已经获胜的 supersession 或 invalidation；
- 需要的临时正文仍为 `AVAILABLE`；
- 披露和上下文预算在消费阶段得到满足。

当前来源绑定只证明本次目标视图；产生某结果的一次性 `ExecutionWorkspaceEvidence` 只证明该历史结果，已清理或隔离的物理目录不需要也不得作为持续权威状态。来源、目标 subject 或相关证据后续失效时不得修改 `AgentFeedbackRecord`，只能发布独立失效事实或由消费资格门拒绝。任何 continuation 都不得绕过披露、上下文预算、来源失效、当前来源绑定或执行证据完整性检查。
...
6. 给定当前来源绑定要求 `SourceBindingRequirement = FORBIDDEN`，发布方不得自行附加当前来源绑定；要求为 `REQUIRED` 时，缺失或无效的当前来源绑定必须阻止发布或消费。
...
提交前必须重新验证当前 run lifecycle、phase-entry、`AgentTurnSubject`、Manifest、candidate、当前 Agent 来源绑定、反馈来源与目标 scope、continuation／supersession／invalidation、正文可用性、DisclosureGrant、上下文与调用预算、凭据状态、单活动 turn 和优先取消；选中反馈来自文件工具结果时，还必须复验产生该结果的一次性 `ExecutionWorkspaceEvidence` 与 cleanup result 摘要。任一条件变化时整笔事务失败并重新计算，不得发布部分 projection、manifest、reservation、turn 或 attempt。已清理的物理目录不参与复验。
