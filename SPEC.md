...
发生合同冲突时，3.1 是规范词、共同处理顺序、错误信封、摘要绑定、状态隔离和确定性要求的权威来源；对象定义由 3.1.6 的权威合同索引指定章节唯一给出。其他章节只能引用或收紧共同规则，不得重新定义或放宽。
...
- 输入及其 Schema 版本；
- 前置 `RunState` 或独立功能状态；
- 适用的运行、阶段、快照、Manifest、候选修订、策略和控制面摘要绑定；
- 规范化与校验顺序；
- 控制面行为及允许产生的副作用；
- 输出、权威记录和生命周期触发事件；
- 边界条件、错误码、重试处置和副作用状态；
...
任何可能触发状态转换、审批消费、工具执行、外部披露或权威写入的结构化请求，均必须按以下顺序处理。各动作合同必须声明适用的绑定集合；某一绑定不适用时必须显式声明，不得静默省略。

1. 按封闭 Schema 和版本解析输入，拒绝未知字段、错误主体和不允许的联合分支。
2. 按对象合同执行版本化规范化，形成规范输入摘要或 `canonical_event_digest`。
3. 对运行内事件执行 `(run_id, event_id)` 幂等查询和复用冲突检查；尚未形成 `run_id` 的创建命令使用其创建合同定义的独立幂等键。
4. 验证调用主体、运行状态、活动阶段，以及合同要求的运行、阶段进入记录、快照、Manifest、候选修订、策略版本、控制面摘要和 fencing generation 绑定。
5. 检查待处理取消请求、预算和资源上限；若当前为 3.2 定义的取消安全点，必须先执行取消转换，不得继续审批或执行。
6. 执行确定性治理和风险判定；`DENY` 必须失败关闭，不能进入授权消费。
7. 验证并原子消费适用的一次性批准，或验证、建立、更新适用的披露授权。
8. 创建唯一逻辑执行尝试及持久化保存的调度意图；事务提交后才可以开始动作执行。
9. 校验执行结果、完整性证据和适用的后置条件。
10. 原子写入权威结果、审计关联和允许的生命周期事件。

后续合同可以在任一步插入更严格的检查，但不得删除、倒置或绕过此前步骤。外部 LLM、Docker、工具调用和权威工作区写入不得包含在生命周期 CAS 或批准消费事务中。

相同 `event_id` 和相同 `canonical_event_digest` 必须返回首次权威处理结果，即使运行状态、阶段或 generation 此后已经变化；相同 `event_id` 和不同摘要必须返回 `EVENT_ID_REUSE_CONFLICT`。首次出现的事件才可以继续动态绑定检查。幂等查询返回既有结果不重新执行副作用，也不属于重试。

第 1—7 步任一步失败时，不得开始动作执行，因此不得产生候选修订变更、外部披露或权威工作区写入。第 8 步开始后发生错误时，结果必须记录：

```text
SideEffectStatus = NONE | COMMITTED | UNKNOWN
```

- `NONE`：控制面能够证明未发生合同定义的副作用。
- `COMMITTED`：控制面能够证明合同定义的副作用已完成。
- `UNKNOWN`：证据不足以证明副作用未发生或已完整发生。

`UNKNOWN` 不得折算为失败后安全重试，也不得折算为成功。仅当动作合同声明幂等，或控制面能够证明 `side_effect_status = NONE` 时，才可以自动重试。一次性批准在执行尝试开始后不得自动恢复为未消费状态。外部披露请求即使超时或无响应，也必须保留披露授权记录和披露尝试记录。
...
运行级错误必须携带当前 `RunPhase` 并禁止 `resource_operation_kind`；可以在来源运行终态后执行的资源级操作必须携带封闭的 `resource_operation_kind` 并禁止 `phase`；`RunState` 形成前的创建错误两者均禁止。调用方提供不适用的条件字段必须被 Schema 拒绝，不能忽略或填充“not applicable”。

`error_code`、`StopReason`、检查状态和策略决定属于不同命名空间，不得由相同字符串或说明文本隐式兼任。`retry_disposition` 必须来自封闭类型并由权威合同直接给出，不得从 `sanitized_message` 推断。解析失败、主体错误、陈旧事件、超时、取消、执行失败和结果未知必须使用可区分的错误码。
...
`RetryDisposition` 规定当前非成功结果唯一允许的后续处置类别，不构成执行、重试、对账或创建新运行的授权。实际处置仍必须满足模块合同、生命周期状态、绑定、治理、审批、预算和幂等条件：

- `NO_RETRY`：原失败请求不得再次执行；只能停止、提交语义不同的新请求，或采用模块明确规定的其他非重试路由。
- `SAME_ATTEMPT_REPLAY`：只允许相同 attempt ID、相同规范输入的传输恢复或幂等结果重放，不得形成第二次逻辑执行。
- `NEW_ATTEMPT_ALLOWED`：模块可以在当前运行内创建新 attempt ID，但必须重新检查状态、预算、绑定和治理；原一次性批准不得复用。
- `NEW_RUN_REQUIRED`：当前运行不得通过重试继续；外部条件修复后只能提交新的创建命令。该值不自动创建新运行。
- `RECONCILIATION_REQUIRED`：必须先通过权威对账判定原副作用；对账完成前不得执行原请求或创建替代尝试。它不等同于 `RunStatus = RECOVERY_REQUIRED`。

每个非成功错误信封必须恰好携带一个 `RetryDisposition`，并由稳定错误码、attempt 状态、`side_effect_status` 和模块合同版本确定性派生。共同优先规则为：

```text
side_effect_status = UNKNOWN
→ RECONCILIATION_REQUIRED
→ 若不存在确定性对账合同，则 NO_RETRY

side_effect_status = COMMITTED
→ 首次权威结果完整：只返回首次结果，不得再次执行副作用
→ 权威结果缺失、不完整或矛盾：RECONCILIATION_REQUIRED
→ 不存在确定性对账合同：NO_RETRY
```

`side_effect_status = UNKNOWN` 时，任何可能重新触发原副作用的操作都不得归类为 `SAME_ATTEMPT_REPLAY`。只读查询供应商幂等键、事务日志或执行状态属于对账。错误记录不终止生命周期时，Schema 必须禁止 `stop_reason`；只有形成 3.2 定义的终态转换时才允许并要求相应停止原因。

错误说明必须脱敏，不得包含凭据、秘密派生信息、未经授权的文件内容、原始缺陷描述或低熵敏感定位符。错误结果需要停止运行时，必须同时产生唯一 `StopReason` 映射和 3.2 规定的权威终态记录；创建事务在 `RunState` 形成前失败时不得伪造 `StopReason`。
...
### 3.2.2 生命周期事件、CAS 与幂等

每个运行记录必须包含从 0 开始单调递增的 `lifecycle_revision`。除创建事务外，每个状态事件必须绑定：
...
LifecycleEventBinding {
  ...
  expected_status
  expected_phase?
  expected_phase_entry_ref?
  expected_lifecycle_revision
  workspace_fencing_generation?
}
```

状态和阶段变更只能由控制面以 CAS 原子提交。成功转换必须同时写入新状态、递增后的修订号、阶段进入或终态记录，以及唯一 `LifecycleTransitionRecord`。

幂等作用域固定为 `(run_id, event_id)`：

- 相同 `event_id` 和相同规范事件摘要的重放必须返回首次结果，不得再次递增修订号、创建第二条逻辑转换记录或重复副作用。
- 相同 `event_id` 和不同规范事件摘要必须返回 `EVENT_ID_REUSE_CONFLICT`，不得改变状态。
- 不同 `event_id` 绑定旧状态、旧阶段、旧阶段进入记录、旧修订号或旧 fencing generation 时，必须形成 `IGNORED_STALE_EVENT`，不得覆盖当前状态。
- 结构或 Schema 无效时必须返回对应解析错误；当前状态匹配但不存在该转换时必须返回 `ILLEGAL_LIFECYCLE_TRANSITION`。二者不得伪装为陈旧事件。

两个具有不同 `event_id` 的事件同时声明预期修订号 `n` 时，最多一个可以转换成功；另一个必须成为陈旧事件，且不得产生其目标转换或副作用。
...
只有 `RUNNING` 可以调度 LLM、工具、检查和持久化尝试。外部长时执行不得位于生命周期事务中。产生执行的转换必须按以下边界处理：

```text
关闭适用等待并记录决定
→ 消费适用的一次性批准，或建立／更新适用的披露授权
→ 创建唯一逻辑执行尝试
→ 写入持久化保存的调度意图
→ 创建新阶段进入记录并进入 RUNNING
→ 事务提交后由执行协调器领取尝试
```

只有确实立即产生外部执行的批准路由才可以创建执行尝试；纯粹派生 Manifest 或更新授权状态的路由不得虚构执行尝试。调度领取和结果接受必须重新校验当前状态、阶段进入记录、修订号和 fencing generation。
...
同一运行最多存在一个活动 `WaitContext`。只有在动作间安全点、不存在正在执行的 LLM、工具或检查租约、被决定对象已经冻结且无有效待处理取消时，才可以进入 `WAITING_USER`。

一个 `wait_id` 在首次被控制面接受、生成 `WaitResolutionRecord` 并原子关闭等待后不得再次消费。仅完成 Schema 解析但未通过主体、绑定或 CAS 的决定不得关闭或消费等待。每个被接受结果必须记录来源主体和规范结果；审批、超时和取消竞态只由控制面持久化顺序和 CAS 决定，不依据客户端时间戳事后猜测。

`TIMEOUT` 只能由控制面使用注入时钟生成。用户只能提交取消请求；`CANCEL_ACCEPTED` 只能由控制面在安全点产生。用户伪造这些结果必须被主体或 Schema 校验拒绝。

v1 的配置修复和凭据录入不属于运行内等待。预检发现缺失配置或凭据时进入 `STOPPED`；用户通过 3.11 的独立功能修复后创建新运行。新增等待类型必须通过版本化 Schema 和 3.9 的具体动作合同显式定义，不得复用现有类型。
...
- 取消过晚：运行已终态或已越过不可逆提交线，只生成忽略记录。

`CREATED`、`WAITING_USER` 和各阶段动作之间的边界是取消安全点。不可中断执行区间中的取消必须保持待处理；到达下一个安全点后优先于普通推进。`RECOVERY_REQUIRED` 不得因取消请求直接停止，必须先解决持久化事务状态。
...
持久化越过不可逆提交线后到达的取消属于过晚事件，不能把已完成的权威写入改写成取消成功。

### 3.2.6 停止原因与终态

`SUCCEEDED` 和 `STOPPED` 是不可逆终态。终态后的审批、取消、执行结果和恢复结果只能生成幂等重放、陈旧或终态忽略记录，不得回退状态。

`StopReason` 与生命周期状态正交。v1 至少使用以下停止原因：
...
`DEMO_COMPLETED` 只允许用于 `run_mode = DEMO`，不得生成正式 `SuccessRecord`、持久化批准或权威工作区写入。正式运行只有 Harness 在独立验证、精确批准、事务持久化和写后核对全部成功后才可以进入 `SUCCEEDED`。

### 3.2.7 正常转换矩阵

下表是正常生命周期转换的唯一规范来源。表中“原子动作”只包括控制面状态和记录提交；外部执行遵循 3.2.3。

| 源状态／阶段 | 触发事件 | 主要守卫 | 原子控制面动作 | 目标状态／阶段 |
| --- | --- | --- | --- | --- |
...
| `WAITING_USER(CONFIRM_REPRODUCTION_APPROVAL)` | `APPROVE` | 主体、等待、动作、补丁、目标、匹配器、Manifest v1、环境和两阶段试验绑定完全匹配 | 关闭等待；消费批准；创建复现执行尝试和调度意图；创建新阶段进入记录 | `RUNNING(REPRODUCTION)` |
...
| `WAITING_USER(DISCLOSURE_AUTHORIZATION)` | `APPROVE` | 决定与供应商、端点、模型、数据范围、预算和有效期完全匹配 | 关闭等待；建立或更新 `DisclosureGrant`；必要时创建 LLM 调用尝试及调度意图；创建新阶段进入记录 | `RUNNING(来源阶段)` |
...
| `RUNNING(AGENT_LOOP)` | `PROPOSE_COMPLETION_ACCEPTED` | `ProposeCompletionAction` 绑定当前候选和 Manifest；预算有效；位于安全点 | 冻结正式验证候选；创建新阶段进入记录和验证调度意图 | `RUNNING(FORMAL_VALIDATION)` |
| `RUNNING(FORMAL_VALIDATION)` | `FORMAL_VALIDATION_FAILED` | 3.10 形成结构化失败反馈；仍有预算且可以继续 | 封存验证结果；创建新阶段进入记录 | `RUNNING(AGENT_LOOP)` |
| `RUNNING(FORMAL_VALIDATION)` | `FORMAL_VALIDATION_PASSED` | 全新副本中的独立正式验证通过；候选和绑定未变化 | 封存验证证据；创建最终批准等待 | `WAITING_USER(FINAL_PERSISTENCE_APPROVAL)` |
| `WAITING_USER(FINAL_PERSISTENCE_APPROVAL)` | `APPROVE` | 决定精确绑定当前 `FinalDiff`、候选树、Manifest 和验证证据；权威工作区前映像仍匹配 | 关闭等待；消费批准；创建唯一持久化尝试、提交意图和新阶段进入记录 | `RUNNING(PERSISTENCE)` |
| 同上 | `APPROVE` | 决定绑定有效，但权威工作区前映像已变化 | 关闭等待；不消费为成功持久化批准；创建变化证据和 `StopRecord` | `STOPPED(WORKSPACE_CHANGED)` |
...
| 任一正式非终态，3.10 允许的检查点 | 权威工作区身份或前映像绑定失效 | `WORKSPACE_CHANGED` |
| 任一非终态 | 预算或资源硬上限耗尽 | `BUDGET_EXHAUSTED` |
| 任一非终态 | 控制面不变量、存储或协议发生不可恢复错误 | `INTERNAL_ERROR` |
| 演示运行的 3.12 规定阶段 | 脚本化演示完成并已重置或进入安全清理 | `DEMO_COMPLETED` |

不得使用无守卫的通配终止事件绕过阶段合同。非持久化动作产生 `side_effect_status = UNKNOWN` 时不自动进入 `RECOVERY_REQUIRED`；其权威合同必须根据风险和可协调能力，明确选择保持或恢复 `RUNNING`、进入 `WAITING_USER` 或进入 `STOPPED`，后续不得假定该动作已成功或已失败。

### 3.2.9 持久化恢复

`RECOVERY_REQUIRED` 只用于 3.10 的权威工作区持久化事务出现不确定或矛盾状态，不泛化为 LLM、披露、Docker 或普通工具失败。

进入该状态后，控制面必须完成事务对账。用户取消、进程重启或普通错误不得跳过对账并直接进入 `STOPPED`。只要事务结果仍不确定，运行必须保持 `RECOVERY_REQUIRED`。

3.10 必须输出结构化 `RecoveryDisposition`，生命周期映射为：

| `RecoveryDisposition` | 守卫与控制面动作 | 目标状态 |
| --- | --- | --- |
| `COMMITTED_AND_VALID` | 证明提交完整、写后摘要一致、全部成功条件仍成立 | `SUCCEEDED` |
| `NOT_COMMITTED_OR_ROLLED_BACK_CONTINUE` | 证明未提交或已完整回滚，且同一授权执行尝试仍可继续 | `RUNNING(PERSISTENCE)` |
| `REVALIDATION_REQUIRED` | 原授权尝试不能继续，候选仍可重新验证 | `RUNNING(FORMAL_VALIDATION)` |
| `REAPPROVAL_REQUIRED` | 验证证据仍有效但必须重新批准精确对象 | `WAITING_USER(FINAL_PERSISTENCE_APPROVAL)` |
| `SAFELY_CLOSED_TERMINATE` | 事务已安全关闭但不能成功，或存在可接受的有效取消 | `STOPPED(RECOVERY_TERMINATED | USER_CANCELLED)` |
| `UNRESOLVED` | 生成新 `RecoveryAttemptRecord` 和新恢复上下文，递增修订号 | `RECOVERY_REQUIRED` |

`UNRESOLVED` 是正式自转换。恢复成功例外路径：

```text
RECOVERY_REQUIRED -> SUCCEEDED
```

只能由恢复证据证明持久化完整、写后摘要一致且成功条件仍成立。正常成功的唯一直接前驱保持为：

```text
RUNNING(PERSISTENCE) -> SUCCEEDED
```

应用启动或运行恢复时，对处于 `RUNNING(PERSISTENCE)` 且发现未关闭、矛盾或结果无法证明的持久化事务，控制面必须通过恢复发现事件进入 `RECOVERY_REQUIRED`；已处于 `RECOVERY_REQUIRED` 的运行必须保持该状态并继续对账。
...
1. 给定合法创建命令，系统必须只创建一个 `CREATED(revision=0)`；同键同载荷重放不得创建第二个运行。
...
6. 给定有效 `ProposeCompletionAction`，系统必须进入新的 `FORMAL_VALIDATION` 阶段实例，且不得生成批准、持久化或成功记录。
7. 给定正式验证失败和可用预算，系统必须进入新的 `AGENT_LOOP` 阶段实例，并把结构化反馈绑定该新实例。
8. 给定同一等待上的批准与取消，最多一个事件可以关闭当前等待；批准先胜出后，迟到取消只能在新运行状态的下一安全点生效。
9. 给定执行已经发出且副作用未知，系统不得把它标记为 `NONE`、自动恢复一次性批准或无条件重试。
10. 给定持久化事务证据矛盾，系统必须进入或保持 `RECOVERY_REQUIRED`，不得以取消或进程重启跳过对账。
11. 给定 `UNRESOLVED` 恢复结果，系统必须递增修订号并生成新的恢复上下文；旧恢复事件随即成为陈旧事件。
12. 给定终态运行的迟到审批、取消或执行结果，系统只能返回重放或忽略结果，不得改变终态。
13. 给定演示完成事件，系统必须进入 `STOPPED(DEMO_COMPLETED)`，不得生成正式 `SuccessRecord`。
