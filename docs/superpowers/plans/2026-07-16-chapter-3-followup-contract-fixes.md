...
affected_contract: 3.5.2 要求 RUNNING(AGENT_LOOP) 的 RepairTurnSubject 绑定当前 CandidateRevision、CandidateTree 和 repair base；只有 `EXISTING_FAILURE_BASELINE_ACCEPTED` 与 `REPRODUCTION_CONFIRMED` 这两个首次 repair-base 转换需要发布初始候选，`FORMAL_VALIDATION_FAILED → AGENT_LOOP` 则必须保留当前候选
counterexample: 若把任意进入 AGENT_LOOP 都解释为初始发布，正式验证失败回环会覆盖或重复当前 CandidateRevision／CandidateTree；若两个首次 repair-base 转换不发布初始候选，第一个 RepairTurnSubject 又无法合法绑定候选
minimal_fix: 3.6 必须只允许 `EXISTING_FAILURE_BASELINE_ACCEPTED` 与 `REPRODUCTION_CONFIRMED` 在首个 RepairTurnSubject 可见前全有或全无地唯一发布初始 CandidateRevision 及 CandidateTree，并使其精确绑定当前 Manifest、phase-entry 和 repair base；必须明确排除 `FORMAL_VALIDATION_FAILED → AGENT_LOOP`，该回环保留当前候选，只发布 FormalValidationResult 与新 phase-entry，由 3.5 确定性生成下一 turn feedback
```

这是 **3.6 的强制前置合同**，不是本计划要在 3.5 中实现的第七项修复。未来 3.6 至少必须冻结：

- 只有 `EXISTING_FAILURE_BASELINE_ACCEPTED` 与 `REPRODUCTION_CONFIRMED` 可以发布初始候选，其他进入或重入 `AGENT_LOOP` 的事件均不得触发初始发布；
- `EXISTING_FAILURE_BASELINE_ACCEPTED` 发布的初始候选内容等于 ExistingFailure 的 `SnapshotTree` repair base；
- `REPRODUCTION_CONFIRMED` 发布的初始候选内容等于 NaturalLanguageDefect 已确认的 `reproduction_candidate_tree` repair base；
- 上述两个首次 repair-base 转换的权威结果在初始 `CandidateRevision`、`CandidateTree`、Manifest、repair base 与 phase-entry 未全有或全无绑定时不得可见；
- `FORMAL_VALIDATION_FAILED → AGENT_LOOP` 明确排除初始发布：保留当前 `CandidateRevision`／`CandidateTree`，只封存 `FormalValidationResult` 与新 phase-entry；3.5 必须据此确定性生成下一 turn feedback，后续 `RepairTurnSubject` 继续绑定该当前候选；
- 初始候选是正式修订，不得用空值、`NO_CANDIDATE` 或占位修订代替；
- 同一首次 repair-base 转换最多发布一个初始候选；发布失败不得留下可启动首个 `RepairTurnSubject` 的阶段状态。
...
`side_effect_status` 必须相对于表中“副作用判定对象”解释。LLM 调用类错误只描述“本次 LLM 适配器调用所封装的下游请求效果”；adapter 方法进入或 Mock `call_count` 增加只是控制流事实，不证明该下游请求效果为 `COMMITTED`，没有下游收据时必须保持 `UNKNOWN`。pre-dispatch commit 中已经提交的 `DisclosureRecord`、披露预算和 feedback 消费由各自权威事实表达，不得因为 `LLM_ADAPTER_CALL_FAILED` 的下游请求效果为 `UNKNOWN` 而回退。动作类错误只描述 `AgentAction` 执行，不把此前已完成的 LLM 调用混入该字段。`COMMITTED` 只证明被引用操作已发生，不表示输出有效或运行成功。
...
实际完成提交：`28f2734b8409cb66b9e2b2dddf0d569fb122ddae`。
...
实际完成提交：`5ffac852808991a421f8a5ab0c507bccef401e91`；后续权威边界收口：`c7f582b6`。
...
- [x] **Step 3: 重命名 3.2.7 并以封闭源状态增加正式重启行**
...
3.2.7 只拥有它明确列出的转换行。3.2.8 拥有取消与跨阶段终止的通用规则和守卫；3.2.7 表内 `CANCEL_ACCEPTED` 行只承载相应状态转换，不得重实现 3.2.8 规则。3.2.9 独占 `RecoveryDisposition` 目标状态映射，3.2.7 不得复制该映射。所有 `RunState` 目标状态映射仍只在 3.2 定义；3.10 只提供领域事实与守卫，不得另行定义生命周期转换。
```
...
- [x] **Step 6: 把 US-08 与 INVEST 改为四类可见状态**
...
- [x] **Step 7: 把 US-06 收窄到 v1 实际批准对象**
...
- [x] **Step 8: 运行生命周期、权威归属和用户故事检查**

```powershell
$spec = Get-Content SPEC.md -Raw
$mapping = [regex]::Match($spec, '(?s)### 3\.2\.6.*?(?=### 3\.2\.7)').Value
$matrix = [regex]::Match($spec, '(?s)### 3\.2\.7.*?(?=### 3\.2\.8)').Value
$cancellation = [regex]::Match($spec, '(?s)### 3\.2\.8.*?(?=### 3\.2\.9)').Value
$recovery = [regex]::Match($spec, '(?s)### 3\.2\.9.*?(?=### 3\.2\.10)').Value
$validation = [regex]::Match($spec, '(?s)### 3\.2\.11.*?(?=## 3\.3)').Value
$us06 = [regex]::Match($spec, '(?s)### US-06.*?(?=### US-07)').Value
$us08 = [regex]::Match($spec, '(?s)### US-08.*?(?=### US-09)').Value
$us06Invest = [regex]::Match($spec, '(?m)^\| US-06 \|.*$').Value
...
function Assert-LifecycleAuthorityPartition(
  [string]$main,
  [string]$cancellationSection,
  [string]$recoverySection
) {
  if ($main -match '下表是运行生命周期转换的唯一规范来源' -and
      $main -notmatch '(?s)下表是运行生命周期转换的唯一规范来源.{0,800}这里的.{0,8}唯一.{0,8}按触发事件归属.{0,400}3\.2\.8.{0,300}3\.2\.9') {
    throw '3.2.7 still makes an unbounded lifecycle-authority claim'
  }
  if ($main -notmatch '(?s)((只|仅).{0,100}(表中|表内|下表|列出).{0,100}(事件|转换|权威)|(表中|表内|下表|列出).{0,100}(只|仅|权威)|这里的.{0,8}唯一.{0,8}按触发事件归属.{0,300}3\.2\.7.{0,160}表中所列)') {
    throw '3.2.7 does not limit its authority to listed table events'
  }
  if ($cancellationSection -notmatch '取消' -or $cancellationSection -notmatch '跨阶段终止') {
    throw '3.2.8 does not own cancellation and cross-phase termination'
  }
  if ($recoverySection -notmatch 'RecoveryDisposition' -or
      $recoverySection -notmatch '(?s)(独占.{0,100}RecoveryDisposition|RecoveryDisposition.{0,100}(独占|唯一|只由)|3\.2\.9.{0,240}是.{0,120}恢复目标状态映射的权威.{0,180}(本表|3\.2\.7).{0,100}不复制)') {
    throw '3.2.9 does not exclusively own RecoveryDisposition target mapping'
  }
  if ($main -match '(?m)^\|[^\r\n]*RecoveryDisposition[^\r\n]*\|') {
    throw '3.2.7 duplicates a RecoveryDisposition target-mapping row'
  }
}

if ([string]::IsNullOrWhiteSpace($cancellation) -or [string]::IsNullOrWhiteSpace($recovery)) {
  throw '3.2.8 or 3.2.9 section is missing'
}
Assert-LifecycleAuthorityPartition $matrix $cancellation $recovery
$badAuthorityFixtures = @(
  [pscustomobject]@{ Main = "### 3.2.7 权威生命周期转换矩阵`n下表是运行生命周期转换的唯一规范来源。"; Cancellation = $cancellation; Recovery = $recovery },
  [pscustomobject]@{ Main = $matrix + "`n| RecoveryDisposition | RETRY_FROM_AGENT_LOOP | RUNNING(AGENT_LOOP) |"; Cancellation = $cancellation; Recovery = $recovery }
)
foreach ($fixture in $badAuthorityFixtures) {
  $rejected = $false
  try { Assert-LifecycleAuthorityPartition $fixture.Main $fixture.Cancellation $fixture.Recovery } catch { $rejected = $true }
  if (-not $rejected) { throw 'synthetic lifecycle-authority violation was accepted' }
}
...
实际完成提交：`695626623742ff73e8cf3d5f73908c416c4b95bf`、`196dbac6bff83f14b993caafab0c03200ad50efe`；后续证明／权威修正：`7a55641d`、`c7f582b6`。
...
- [x] **Step 2: 统一 `DisclosureRecord` 的语义**
...
- [x] **Step 5: 冻结 checkpoint 后崩溃语义和 Mock 例外**
...
实际完成提交：`d6c4d4455e7a1cc1757f2a7e39f3501c5f3319b7`；HANDOFF 完整路线：`fc09b05e`。

**Files:**

- Modify: `SPEC.md` — 3.2.6、3.5.1、3.5.3—3.5.5、3.5.7—3.5.8

- [x] **Step 1: 证明当前 `LLMCallFailureRecord` 路由缺失**
...
- [x] **Step 3: 在 3.5.8 增加带副作用状态的最小错误路由表**
...
必须解释：LLM 调用类错误的 `side_effect_status` 唯一相对于“本次 LLM 适配器调用所封装的下游请求效果”；adapter 方法进入或 Mock `call_count` 增加只是控制流事实，不证明该效果为 `COMMITTED`，没有下游收据时仍为 `UNKNOWN`。动作类错误相对于 `AgentAction` 执行。pre-dispatch commit 中已经提交的 `DisclosureRecord`、披露预算和 feedback 消费由其权威记录表达，不因该下游请求效果为 `UNKNOWN` 而回退。

- [x] **Step 4: 在 3.5.4 补全完成类别映射**
...
$routeSectionMatch = [regex]::Match(
  $spec,
  '(?ms)^#{1,6}\s+3\.5\.8(?:\s|$).*?(?=^#{1,6}\s+3\.5\.9(?:\s|$))'
)
if (-not $routeSectionMatch.Success) { throw '3.5.8 error route section is not uniquely bounded' }

function Assert-MinimalErrorRouteTable([string]$section) {
  $expected = @(
    'mandatory context 规范压缩后仍超预算 || CONTEXT_BUDGET_EXCEEDED || 拟议 LLM 适配器调用 || NONE || 不创建 AgentTurn、不伪造完成类别 || NEW_RUN_REQUIRED || STOPPED(BUDGET_EXHAUSTED)',
    '模型输出 Schema 无效、未达阈值 || MODEL_OUTPUT_INVALID || 本次 LLM 适配器调用 || COMMITTED || RejectedTurnOutputRecord + OUTPUT_INVALID + CREATE_NEXT_TURN || NEW_ATTEMPT_ALLOWED || 当前运行非终态',
    '模型输出 Schema 无效、达到阈值 || MODEL_OUTPUT_INVALID_LIMIT_REACHED || 本次 LLM 适配器调用 || COMMITTED || RejectedTurnOutputRecord + OUTPUT_INVALID + RUN_STOPPED || NEW_RUN_REQUIRED || STOPPED(NO_PROGRESS)',
  ...
  foreach ($line in ($section -split '\r?\n')) {
    ...
    $cells = @($line.Trim().Trim('|').Split('|') | ForEach-Object {
      (($_ -replace '`', '') -replace '\s+', ' ').Trim()
    })
    if ($cells[0] -eq '场景' -or $cells[0] -match '^-+$') { continue }
    if ($cells.Count -ne 7) { throw "3.5.8 route row must have 7 columns: $line" }
    $keys += "$($cells[0]) || $($cells[1])"
    $actual += ($cells -join ' || ')
  }
  if ($actual.Count -ne 9) { throw "3.5.8 must contain exactly 9 data rows; got $($actual.Count)" }
  if (@($keys | Select-Object -Unique).Count -ne 9) {
    throw '3.5.8 route rows must be unique by scenario + error_code'
  }
  foreach ($row in $expected) {
    if (@($actual | Where-Object { $_ -ceq $row }).Count -ne 1) {
      throw "missing or duplicated exact 3.5.8 route row: $row"
    }
  }
}

Assert-MinimalErrorRouteTable $routeSectionMatch.Value
foreach ($proof in @(
  '本次 LLM 适配器调用所封装的下游请求效果',
  '(?:进入\s*adapter\s*方法|方法调用开始)',
  'Mock.{0,80}call_count',
  '只证明.{0,40}控制流.{0,80}不得据此.{0,120}COMMITTED',
  '没有.{0,30}收据.{0,160}UNKNOWN'
)) {
  if ($routeSectionMatch.Value -notmatch $proof) {
    throw "3.5.8 LLM receipt boundary missing: $proof"
  }
}
$wrongObjectFixtures = @(
  $routeSectionMatch.Value.Replace(
    '本次 LLM 适配器调用所封装的下游请求效果',
    '本次 LLM 适配器调用'
  ),
  $routeSectionMatch.Value.Replace(
...
foreach ($fixture in $wrongObjectFixtures) {
  $rejected = $false
  try { Assert-MinimalErrorRouteTable $fixture } catch { $rejected = $true }
  if (-not $rejected) { throw 'synthetic wrong LLM side-effect object was accepted' }
}
...
实际完成提交：`67d0b5b79fc511f440f19e52f3d30f0f5c9b3acd`。
...
实际完成提交：`06b08a94c142db68bad70e57dd38985b17d9a441`；交接／当前权威同步：`fc09b05e`、`79d3ae46`、`c7f582b6`、`a31922c8`。这些状态只记录历史实施完成，不表示当前候选已经通过 Task 7、独立审查或最终日志门禁；Task 7 的审查／日志状态以最终 `AGENT_LOG.md` 与 `TASK_HANDOFF.md` 为准。一旦固定送审内容 SHA，不得再修改本计划 checkbox，否则既有审查失效；Task 7 在实际完成前必须保持全部未勾选。
...
- [x] **Step 1: 在 `SPEC_PROCESS.md` 追加原审阅与执行前方案复审证据**
...
- [x] **Step 2: 更新 `TASK_HANDOFF.md` 的当前合同**
...
- [x] **Step 3: 在交接材料冻结 3.5 锁定边界和 3.6 初始候选义务**
...
3.6 只允许 EXISTING_FAILURE_BASELINE_ACCEPTED 与 REPRODUCTION_CONFIRMED 在首个 RepairTurnSubject 可见前全有或全无地唯一发布初始 CandidateRevision 和 CandidateTree；FORMAL_VALIDATION_FAILED → AGENT_LOOP 明确不触发初始发布并保留当前候选。
```

3.6 的验收问题必须覆盖两个首次 repair-base 事件（`EXISTING_FAILURE_BASELINE_ACCEPTED` 与 `REPRODUCTION_CONFIRMED`）、初始候选全有或全无发布、单一胜出、禁止空／占位候选，以及发布失败时不得进入可创建首个修复 turn 的 `AGENT_LOOP` 状态；还必须明确拒绝 `FORMAL_VALIDATION_FAILED` 初始发布，验证该回环保留当前 `CandidateRevision`／`CandidateTree`，只发布 `FormalValidationResult` 与新 phase-entry，由 3.5 生成下一 turn feedback，并使后续 `RepairTurnSubject` 继续绑定当前候选。
...
```powershell
$plan = Get-Content docs/superpowers/plans/2026-07-16-chapter-3-followup-contract-fixes.md -Raw
$handoff = Get-Content TASK_HANDOFF.md -Raw
$spec = Get-Content SPEC.md -Raw
$planAuthority = [regex]::Replace($plan, '(?ms)^```powershell[^\r\n]*\r?\n.*?^```\s*$', '')
$futureGateAuthority = "$planAuthority`n$handoff"

foreach ($event in 'EXISTING_FAILURE_BASELINE_ACCEPTED','REPRODUCTION_CONFIRMED') {
  if ($futureGateAuthority -notmatch $event) { throw "future 3.6 initial-candidate event missing: $event" }
}
if ($futureGateAuthority -match '每次进入\s*`?AGENT_LOOP`?.{0,120}(发布|初始)') {
  throw 'future 3.6 gate still publishes an initial candidate on every AGENT_LOOP entry'
}
if ($futureGateAuthority -notmatch '(?s)FORMAL_VALIDATION_FAILED.{0,160}(不触发|排除|不得).{0,100}初始发布') {
  throw 'future 3.6 gate does not explicitly exclude formal-validation re-entry'
}
foreach ($term in '保留当前','FormalValidationResult','phase-entry','下一 turn feedback','RepairTurnSubject') {
  if ($futureGateAuthority -notmatch [regex]::Escape($term)) {
    throw "future 3.6 formal-validation loop relation missing: $term"
  }
}
if ($spec -notmatch '(?s)FORMAL_VALIDATION_FAILED.{0,500}AGENT_LOOP') {
  throw 'current SPEC lacks the formal-validation failure re-entry used by the future 3.6 gate'
}

function Assert-InitialCandidateEventGate(
  [string[]]$initialEvents,
  [bool]$formalPublishesInitial,
  [bool]$formalPreservesCurrent,
  [bool]$formalPublishesResult,
  [bool]$formalPublishesPhaseEntry,
  [bool]$feedbackFeedsNextTurn,
  [bool]$nextSubjectBindsCurrent
) {
  $expected = @('EXISTING_FAILURE_BASELINE_ACCEPTED','REPRODUCTION_CONFIRMED')
  if (@(Compare-Object $expected $initialEvents).Count -ne 0) {
    throw 'initial candidate gate must allow exactly the two first repair-base events'
  }
  if ($formalPublishesInitial) { throw 'FORMAL_VALIDATION_FAILED must not publish an initial candidate' }
  if (-not ($formalPreservesCurrent -and $formalPublishesResult -and
            $formalPublishesPhaseEntry -and $feedbackFeedsNextTurn -and
            $nextSubjectBindsCurrent)) {
    throw 'formal-validation re-entry relation is incomplete'
  }
}

$good = @{
  initialEvents = @('EXISTING_FAILURE_BASELINE_ACCEPTED','REPRODUCTION_CONFIRMED')
  formalPublishesInitial = $false
  formalPreservesCurrent = $true
  formalPublishesResult = $true
  formalPublishesPhaseEntry = $true
  feedbackFeedsNextTurn = $true
  nextSubjectBindsCurrent = $true
}
Assert-InitialCandidateEventGate @good
$badEvents = $good.Clone()
$badEvents.initialEvents = @('EXISTING_FAILURE_BASELINE_ACCEPTED','REPRODUCTION_CONFIRMED','FORMAL_VALIDATION_FAILED')
$badPublication = $good.Clone()
$badPublication.formalPublishesInitial = $true
$badRetention = $good.Clone()
$badRetention.formalPreservesCurrent = $false
foreach ($fixture in $badEvents,$badPublication,$badRetention) {
  $rejected = $false
  try { Assert-InitialCandidateEventGate @fixture } catch { $rejected = $true }
  if (-not $rejected) { throw 'synthetic invalid future 3.6 gate was accepted' }
}
'PASS'
```
...
$currentPlanAuthority = [regex]::Replace(
  $planText,
  '(?ms)^```powershell[^\r\n]*\r?\n.*?^```\s*$',
  ''
)
...
$requiredAuthority = [ordered]@{
  ...
  lifecycle_table_event_owner = '(?s:3\.2\.7.{0,800}(?:(?:只|仅).{0,160}(?:表中|表内|下表|列出).{0,120}(?:事件|转换|权威)|这里的.{0,8}唯一.{0,8}按触发事件归属.{0,300}3\.2\.7.{0,160}表中所列))'
  cancellation_termination_owner = '(?s:3\.2\.8.{0,360}(?:取消.{0,100}跨阶段终止|跨阶段终止.{0,100}取消))'
  recovery_disposition_owner = '(?s:3\.2\.9.{0,600}(?:独占.{0,120}RecoveryDisposition|RecoveryDisposition.{0,120}(?:独占|唯一|只由)|是.{0,120}恢复目标状态映射的权威.{0,180}(?:本表|3\.2\.7).{0,100}不复制))'
}
