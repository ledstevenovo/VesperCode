...
- 2026-07-16 二次交叉审阅又发现六项问题，其中五项阻断重新打开相关冻结，一项 `NON_BLOCKING_ENHANCEMENT` 只作澄清；对应最小合同修正已经进入当前 `SPEC.md`，仍须接受新的只读审查；
- 3.5 仍只有九个小节，3.5.9 是固定编号 1—21 的最低必测集合；3.5.1—3.5.8 中未逐项重复的规范合同继续有效；
- 仓库仍处于规格阶段，没有权威课程 `PLAN.md` 或实现代码。

3.5 写作结构已完成；本轮四项3.5收口通过审查后可局部锁定。不新增 3.5.10。3.5局部锁定不等于首轮 `RepairTurnSubject` 端到端闭环已验证。3.6 只允许 `EXISTING_FAILURE_BASELINE_ACCEPTED` 与 `REPRODUCTION_CONFIRMED` 在首个 `RepairTurnSubject` 可见前全有或全无地唯一发布初始 `CandidateRevision` 和 `CandidateTree`；`FORMAL_VALIDATION_FAILED → AGENT_LOOP` 明确不触发初始发布并保留当前候选。完整第三章仍未锁定，仍须完成 3.6—3.12 并接受全章交叉审查。
...
发生冲突时，优先级为：课程源文档 → `SPEC.md` 当前合同 → `SPEC_PROCESS.md` 的当前审查规则 → 本交接文档。固定 `83746d7` 后三项只读审查均已通过属于历史事实；2026-07-16 二次交叉审阅新增的五项阻断已重新打开相关冻结。历史过程记录不能覆盖当前规范，四项 3.5 收口只有通过新审查后才可局部锁定，完整第三章仍未锁定。
...
3.5.9 的 21 项是固定最低验收清单，不排除 3.5.1—3.5.8 的明确规范合同。固定 `83746d7` 后的审查结论保留为历史证据；本轮五项阻断重新打开相关冻结，四项 3.5 收口只有通过新的只读审查后才可局部锁定。
