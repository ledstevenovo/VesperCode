...
- 第 3 章的 3.1—3.5；
- 3.1—3.4 的 v1 范围收敛；
- 3.5 的主循环、不可变树文件工具和下一轮反馈协议收敛；
- 3.5 唯一的 21 项固定验收清单；
- 规范符合性审查、文档质量审查和无背景范围冷审。
...
发生冲突时，优先级为：课程源文档 → `SPEC.md` 当前冻结合同 → `SPEC_PROCESS.md` 的当前审查规则 → 本交接文档。历史过程记录不能覆盖当前规范。
...
本地动作策略只有冻结的复现确认动作和最终持久化动作可以进入 `ASK`；披露授权属于独立状态空间。不要重新引入通用 `ACTION_APPROVAL`，除非第 2 章产品承诺被用户显式修改。
...
- 进程重启创建新 lease ref；
- 获取锁后的旧运行失败关闭/持久化恢复门；
- clean Git、敏感路径、`SnapshotTree` 唯一来源、Docker-only 和 Demo 隔离。
...
- 每轮一次 LLM 调用、一个权威输出、恰好一个动作；
- 保留八值 `AgentAction` 判别联合及现有阶段矩阵；
...
- 每条选中反馈恰好一个 reservation，一个 Manifest 聚合完整集合；
- Accepted 和 Rejected 都全量消费；调用失败、输出前取消和内部中止全量关闭但不消费；
- `SAME_ATTEMPT_REPLAY` 复用原 Manifest/reservations；
- 迟到结果不重开终态；
- 响应后不可恢复内部故障停止为 `INTERNAL_ERROR`，且不重新请求供应商。
...
3.5 的细化门禁只来自 3.5.9 已冻结的 21 项清单。
...
以及 SPEC_PROCESS.md 的 10.7 和第 11 节。
