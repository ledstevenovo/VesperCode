...
3.5 写作结构已完成；本轮四项3.5收口通过审查后可局部锁定。不新增 3.5.10。3.5局部锁定不等于首轮 `RepairTurnSubject` 端到端闭环已验证。3.6 必须在任何首个 `RepairTurnSubject` 可见前发布唯一初始 `CandidateRevision` 和 `CandidateTree`。完整第三章仍未锁定，仍须完成 3.6—3.12 并接受全章交叉审查。
...
3.6 的首要下游门禁已经固定：ExistingFailure 与 NaturalLanguageDefect 两类 repair base 都必须在进入可创建 turn 的 `AGENT_LOOP` 前，全有或全无地发布唯一、非空且非占位的初始 `CandidateRevision` 与 `CandidateTree`。并发发布必须单一胜出；发布失败不得进入能够创建首个 `RepairTurnSubject` 的 `AGENT_LOOP`。该门禁属于 3.6 的 `PROVABLY_UNIMPLEMENTABLE` 问题，不是新的 3.5 修复项。
...
- ExistingFailure 与 NaturalLanguageDefect 两类 repair base 分别从哪个冻结事实派生初始候选；
...
- 两类 repair base 的初始候选必须全有或全无地发布，且并发时只有一个权威发布胜出；
- 初始候选不得为空或使用占位引用；发布失败不得进入可创建首个 `RepairTurnSubject` 的 `AGENT_LOOP`；
...
两类 repair base 都必须在任何首个 RepairTurnSubject 可见前，全有或全无地
发布唯一、非空且非占位的初始 CandidateRevision 与 CandidateTree；并发发布
只能单一胜出，发布失败不得进入能够创建首个 turn 的 AGENT_LOOP。
