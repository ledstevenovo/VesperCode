...
- 固定 SHA `48d06ef` 的只读审查发现启动终止未闭合旧 turn 的阻断，当前分支已写入最小修正并等待复审；
...
- 进程重启创建新 lease ref；
- 获取锁后的旧运行失败关闭/持久化恢复门；
- `PROCESS_RESTARTED_DURING_RUN` 的启动终止必须以同一 CAS／权威提交全有或全无地闭合旧运行：已开始且无结果的 `AdmissionCheckAttempt → UNKNOWN`，未开始或无法安全启动的 attempt → `NOT_RUN(CONTROL_PLANE_ABORTED)`，未完成 `AgentTurn → TURN_ABORTED + RUN_STOPPED`；已有 outcome 和已终局对象不得改写；
- 旧 turn 在 checkpoint 前必须闭合 `AgentFeedbackConsumptionManifest` 并全量关闭 reservations 且不消费；checkpoint 后无 outcome 时必须闭合 `AgentFeedbackConsumptionManifest`、保持 reservations 已消费并保留披露／预算／可能已调用事实，不得创建 outcome、调用、恢复或重发适配器；
...
- 3.3.7 的新进程启动终止是既有崩溃关闭例外：它以 `TURN_ABORTED + RUN_STOPPED` 闭合未完成 turn，不创建或伪造 `AgentTurnOutcome`；
...
- owning process 内 checkpoint 前取消或内部中止全量关闭 reservations 且不消费；3.3.7 启动终止在 checkpoint 前执行相同关闭，在 checkpoint 后无 outcome 时保持已消费；
...
- 启动终止不得调用、恢复或重发适配器；旧 process、lease、turn 或 attempt 绑定的迟到结果不得重开或改写终态；
