...
> 权威分支：`origin/main`。
> 本文创建前，`main` 与 `origin/main` 均已合并并指向 `64de2a3846c05b81ad69909a6e59be976e87d2c1`。本文本身将通过后续提交加入 `main`，因此新对话应以最新 `origin/main` 为准，而不是把该前置提交当成最终 HEAD。
...
- 权威分支：`main`。
- Phase 2 合并前置提交：`64de2a3846c05b81ad69909a6e59be976e87d2c1`。
- Phase 2 功能分支：`codex/spec-v1-stage35-reset`，已经快进合并到 `main` 并推送。
- Phase 1 检查点分支：`spec-v1-scope-reset`，最终提交 `1759f0fcb96ee6f6e31fb2e2ee07beebaa832c67`。
- Phase 2 最终冷审范围：`1759f0f..2ace4bc`；随后只有冷审证据日志提交 `64de2a3`，该日志提交又经过冷审代理的窄范围真实性复核。
...
- 旧进程遗留的非终态 Demo 运行在启动时形成 `error_code = DEMO_SESSION_INVALIDATED`、`StopReason = INTERNAL_ERROR` 和完整 `StopRecord`；
...
- 每个逻辑 turn 恰好关联一个 LLM call attempt；dispatch checkpoint 后至多形成一个权威 `AgentTurnOutcome`；
- `AgentTurnOutcome` 是 `AcceptedTurnOutputRecord | RejectedTurnOutputRecord | LLMCallFailureRecord | TurnProcessingFailureRecord` 四值联合；
...
- 披露等待前只能构建非权威 `ContextProjectionDraft`；授权不足只创建 `WaitContext`，不得创建 turn、attempt、feedback、Manifest 或 reservations；授权满足并在新 phase-entry 全量重算后，才原子创建 `AgentTurn`、最终 `ContextProjection`、attempt、适用 feedback、Manifest 和 reservations；
- feedback 只支持 `NEXT_TURN_ONLY` 有界摘要，不跨候选或 phase 迁移；
- 每条选中反馈恰好一个 reservation，一个 Manifest 聚合完整集合；确定性 dispatch checkpoint 在适配器调用前提交并立即全量消费 reservations；
- Accepted、Rejected、`LLMCallFailureRecord` 和 `TurnProcessingFailureRecord` 都保持 checkpoint 后的消费；
- 只有 checkpoint 前取消或内部中止才全量关闭 reservations 且不消费；
- `SAME_ATTEMPT_REPLAY` 只能为相同标识和相同规范输入返回首次已形成的同一 outcome，并复用原最终投影、Manifest 和 reservations；尚无 outcome 时不得重发适配器调用；
- 迟到结果不重开终态；
...
AI4SE_Final_Project_A_Coding_Agent_Harness(1).md、SPEC.md 的 3.1、3.4、3.5，
