# VesperCode 历史任务交接与规格收敛方法论

> **状态：历史归档（非规范）。** 本文保留 2026-07-15 至 2026-07-25 的 SPEC 第三章收敛过程；其中关于 SPEC_v3.md、SPEC 仅完成至 3.5.9、下一任务为“3.6”以及当时 Git 现场的陈述均已过期。
>
> **不得作为当前实施输入。** 陌生 Agent、正式实施 Agent 和冷启动试作只能以课程原文、当前根目录 SPEC.md、当前根目录 PLAN.md、SPEC_PROCESS.md 与 AGENT_LOG.md 为准。本文件不定义现行任务、准入状态、验收标准或工作流。
...
> 本轮二次交叉审阅修正基线为 `53ddefd1676c2c72603dddaac33393ffb3627ef7`，本轮分支为 `codex/ch3-followup-review-plan`。合并后开启新对话时必须先成功 fetch，并以当时最新 `origin/main` 为准，不得把本文中的历史或基线 SHA 当作最终提交。
...
- 第一轮针对 3.1—3.5 交叉审阅发现的 14 项问题已经写入最小合同修正；
- `bd50d82` 闭合正式 3.3.7 启动终止，`a125dfe` 随后完成 Demo 同构修复；固定 `83746d7` 后的规范符合性、文档质量和累计范围无背景冷审均曾实际 `PASS`，这是保留的历史证据；
- 2026-07-16 二次交叉审阅又发现六项问题，其中五项阻断重新打开相关冻结，一项 `NON_BLOCKING_ENHANCEMENT` 只作澄清；对应最小合同修正已经进入当前 `SPEC.md`，固定内容 SHA `1fc0fc4524013b16e51f48c43cbb831f63145e32` 已在 2026-07-17 通过全新候选复验、规范符合性、文档质量和无背景范围冷审；
- 3.5 仍只有九个小节，3.5.9 是固定编号 1—21 的最低必测集合；3.5.1—3.5.8 中未逐项重复的规范合同继续有效；
- 仓库仍处于规格阶段，没有权威课程 `PLAN.md` 或实现代码。

3.5 写作结构已完成；本轮四项 3.5 收口已在固定内容 SHA `1fc0fc4524013b16e51f48c43cbb831f63145e32` 通过三轮独立只读审查，3.1—3.5 已重新锁定。不新增 3.5.10。3.5 局部锁定不等于首轮 `RepairTurnSubject` 端到端闭环已验证。3.6 只允许 `EXISTING_FAILURE_BASELINE_ACCEPTED` 与 `REPRODUCTION_CONFIRMED` 在首个 `RepairTurnSubject` 可见前全有或全无地唯一发布初始 `CandidateRevision` 和 `CandidateTree`；`FORMAL_VALIDATION_FAILED → AGENT_LOOP` 明确不触发初始发布并保留当前候选。完整第三章仍未锁定，仍须完成 3.6—3.12 并接受全章交叉审查。
...
- 权威分支：`main`。
- 第一轮交叉审阅修正的历史基线：`cf720407af69aaac235b2bb0f7923fecd0544c68`。
- 第一轮交叉审阅修正的历史分支：`codex/ch3-review-fixes`。
- 本轮二次交叉审阅修正基线：`53ddefd1676c2c72603dddaac33393ffb3627ef7`。
- 本轮分支：`codex/ch3-followup-review-plan`；合并后仍以成功 fetch 得到的最新 `origin/main` 为唯一新任务起点。
...
发生冲突时，优先级为：课程源文档 → `SPEC.md` 当前合同 → `SPEC_PROCESS.md` 的当前审查规则 → 本交接文档。固定 `83746d7` 后三项只读审查均已通过属于历史事实；2026-07-16 二次交叉审阅新增的五项阻断曾重新打开相关冻结。历史过程记录不能覆盖当前规范；本轮四项 3.5 收口已在固定内容 SHA `1fc0fc4524013b16e51f48c43cbb831f63145e32` 通过新的三轮独立只读审查并局部锁定，3.1—3.5 已重新锁定，完整第三章仍未锁定。
...
所有 `RunState` 目标状态映射均由 3.2 拥有。3.2.7 只拥有它明确列出的转换行。3.2.8 拥有取消与跨阶段终止的通用规则和守卫；3.2.7 表内 `CANCEL_ACCEPTED` 行只承载相应状态转换，不得重实现 3.2.8 规则。3.2.9 独占 `RecoveryDisposition` 目标状态映射，3.2.7 不得复制该映射。正式 `PROCESS_RESTART_DETECTED` 行只覆盖七项非持久化非终态：`CREATED`、`WAITING_USER`、`RUNNING(PREFLIGHT)`、`RUNNING(BASELINE)`、`RUNNING(REPRODUCTION)`、`RUNNING(AGENT_LOOP)` 和 `RUNNING(FORMAL_VALIDATION)`；它必须经 3.2.6 的 `PROCESS_RESTARTED_DURING_RUN → INTERNAL_ERROR` 映射并引用 3.3.7 的对象级闭合细节。`RUNNING(PERSISTENCE)` 与 `RECOVERY_REQUIRED` 明确排除，只能走 3.2.9 的恢复目标状态映射，并由 3.10 提供持久化事务／恢复领域证据、事务事实、`RecoveryDisposition` 与守卫；对应机械验证位于 3.2.11 第 12 项。Demo 启动失效仍使用独立矩阵行，不得取得或伪造正式 lease。
...
- `DisclosureGrant` 只绑定运行级供应商、端点、模型、允许来源与数据类别、脱敏规则、累计预算和有效期；真实路线的既有 `DisclosureRecord` 在调用前 pre-dispatch commit 中绑定最终请求摘要、实际来源、数据类别、规范外发载荷字节数、脱敏结果和所消费 grant；
...
- dispatch checkpoint 后真实或 Mock LLM 适配器终态失败必须形成 `LLMCallFailureRecord`、`LLM_ADAPTER_CALL_FAILED`、`side_effect_status = UNKNOWN`、`LLM_CALL_FAILED + RUN_STOPPED` 和 `STOPPED(EXECUTION_TERMINATED)`；该 `UNKNOWN` 的唯一判定对象是“本次 LLM 适配器调用所封装的下游请求效果”，adapter 方法进入或 Mock `call_count` 增加只是控制流事实，没有下游收据时不得推导 `COMMITTED`；当前运行不得重试或重发，pre-dispatch 已提交事实不回退；
- 3.5.8 的最小错误路由表逐行冻结场景、稳定错误码、副作用判定对象、`side_effect_status`、轮次处置、重试处置和运行路线；`LLM_ADAPTER_CALL_FAILED` 行必须将“本次 LLM 适配器调用所封装的下游请求效果”与 `UNKNOWN` 绑定，其他状态也必须相对于各自行的判定对象解释；
...
3.5.9 的 21 项是固定最低验收清单，不排除 3.5.1—3.5.8 的明确规范合同。固定 `83746d7` 后的审查结论保留为历史证据；本轮五项阻断曾重新打开相关冻结，四项 3.5 收口现已在固定内容 SHA `1fc0fc4524013b16e51f48c43cbb831f63145e32` 通过新的三轮独立只读审查并局部锁定。
...
acceptance_dimension: 七项维度、最低清单编号，或 3.5.1—3.5.8 明确规范合同
...
3.6 的首要下游门禁已经固定：只允许 `EXISTING_FAILURE_BASELINE_ACCEPTED` 与 `REPRODUCTION_CONFIRMED` 这两个首次 repair-base 转换在首个 `RepairTurnSubject` 可见前，全有或全无地发布唯一、非空且非占位的初始 `CandidateRevision` 与 `CandidateTree`。并发发布必须单一胜出；发布失败不得进入能够创建首个 `RepairTurnSubject` 的 `AGENT_LOOP`。`FORMAL_VALIDATION_FAILED → AGENT_LOOP` 明确排除初始发布，必须保留当前候选，只发布 `FormalValidationResult` 与新 phase-entry，由 3.5 生成下一 turn feedback，后续 `RepairTurnSubject` 继续绑定当前候选。该门禁属于 3.6 的 `PROVABLY_UNIMPLEMENTABLE` 问题，不是新的 3.5 修复项。
...
- 为什么只有 `EXISTING_FAILURE_BASELINE_ACCEPTED` 与 `REPRODUCTION_CONFIRMED` 可以分别从 ExistingFailure `SnapshotTree` 与 NaturalLanguageDefect `reproduction_candidate_tree` repair base 派生初始候选；
- 初始 `CandidateRevision` 与 `CandidateTree` 如何全有或全无地发布；
- 两个并发发布如何保证只有一个权威结果胜出；
- 如何机械拒绝空值、缺失引用、占位摘要或占位树；
- 初始发布失败时如何保证系统不会进入可创建首个 `RepairTurnSubject` 的 `AGENT_LOOP`；
- 如何机械拒绝把 `FORMAL_VALIDATION_FAILED` 纳入初始发布，并验证该回环保留当前 `CandidateRevision`／`CandidateTree`，只发布 `FormalValidationResult` 与新 phase-entry，由 3.5 生成下一 turn feedback，使后续 `RepairTurnSubject` 继续绑定当前候选；

通用 CandidateRevision 问题：
...
2. 写出 3.6 的范围、非目标和封闭验收问题，并覆盖两类 repair base 与初始候选发布门禁；
3. 由用户显式批准验收维度；
4. 再写 3.6 正文；
...
- 只有 `EXISTING_FAILURE_BASELINE_ACCEPTED` 与 `REPRODUCTION_CONFIRMED` 可以从各自 repair base 全有或全无地发布初始候选，且并发时只有一个权威发布胜出；
- 初始候选不得为空或使用占位引用；发布失败不得进入可创建首个 `RepairTurnSubject` 的 `AGENT_LOOP`；`FORMAL_VALIDATION_FAILED → AGENT_LOOP` 不得发布初始候选，必须保留当前候选并闭合 `FormalValidationResult`、新 phase-entry、下一 turn feedback 与后续 `RepairTurnSubject` 的关系；
...
以及 SPEC_PROCESS.md 的 10.7、第 11 节、第 12 节和第 13 节。

当前任务只制定并审查 SPEC 3.6 的 v1 范围和封闭验收问题；
只有 EXISTING_FAILURE_BASELINE_ACCEPTED 与 REPRODUCTION_CONFIRMED 可以在首个
RepairTurnSubject 可见前，从各自 repair base 全有或全无地发布唯一、非空且
非占位的初始 CandidateRevision 与 CandidateTree；FORMAL_VALIDATION_FAILED →
AGENT_LOOP 不触发初始发布，必须保留当前候选，只发布 FormalValidationResult
与新 phase-entry，由 3.5 生成下一 turn feedback，后续 RepairTurnSubject 继续
绑定当前候选；并发发布只能单一胜出，首次发布失败不得进入可创建首个 turn 的 AGENT_LOOP。
...
- 哪个本地 worktree 不能触碰；
- `SPEC.md` 已经完成到哪一节；
- 为什么固定 `83746d7` 后三项只读审查通过只是历史证据，以及本轮五项阻断为何重新打开相关冻结；
- 下一任务为什么是 3.6；
- 3.5 的九个小节、固定最低验收清单、七值动作、四值 outcome、真实调用前 checkpoint、`LLM_ADAPTER_CALL_FAILED` 终止和只返回既有结果的 replay 决策是什么；
- 为什么 3.5 的局部锁定不等于首个 `RepairTurnSubject` 已经端到端闭合；
- 3.6 为什么必须为两类 repair base 全有或全无地发布唯一非占位初始候选，并在失败时阻止进入可创建 turn 的 `AGENT_LOOP`；
- 3.9 与 3.5 如何分工，以及 3.12 为什么必须把 `DisclosureRecord` 显示为调用前调度提交而不是供应商送达事实；
- 哪些生产级机制已经明确移出 v1；
