...
- **具体修正：** 3.10 只形成并封存 FormalValidationResult；3.5 在原子创建下一 AgentTurn 时，从该结果确定性生成并绑定有界下一轮反馈。
...
本轮没有增加 RunStatus、RunPhase、WaitKind、StopReason、AgentAction、generation、takeover、reconciliation、persistent block 或供应商送达协议。3.9 仍负责 DisclosureGrant、披露预算账本、DisclosureRecord 发布键／幂等与 pre-dispatch commit 的权威正文；3.5 只规定调用时点和绑定。3.12 后续只能把 DisclosureRecord 展示为“已完成调用前调度提交；适配器调用与交付状态未知”，不得无条件显示“已发送”或“供应商已收到”。本轮不编写 3.9 或 3.12 正文。
