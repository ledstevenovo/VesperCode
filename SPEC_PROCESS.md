...
- **WP33 合入 main**：分支 codex/wp33 → main。E5（WP33 +
  WP34-REFERENCE）收官。
- **WP35 合入 main（SPEC_PROCESS 87）**：分支 codex/wp35 → main（merge commit 1f8fd6c，2026-08-08）。T35.1（里程碑 35 唯一 session task）完成：GitHub Actions 三 job（unit-test/reference-image-build/demo-image-build）在 push 与 PR 上全绿且无发布秘密（引擎 29.1.3 + buildx 0.30.1 固定）；GitLab 四 job 契约（无项目——Real 步诚实跳过，dind 拓扑 loopback 绑定 VESPER_LOOPBACK_BIND_HOST/PROBE_HOST 已本地演练）；fail-closed protected-tag 规则；冻结通过证据（同一 SHA 的 push/PR 运行 URL/状态记录于 PLAN/AGENT_LOG）。
- **SPEC_PROCESS 86 跨平台确定性镜像**（本轮核心）：reference 镜像冻结身份 manifest `cf0b6c5c…`/profile `d0700f00…` 在 Windows formal 环境与 GitHub Linux runner 上字节一致复现。规范化链：成员按名排序（buildkit 保留文件系统写入序——pip 先写 wheel 后写 dist-info）→ 固定 epoch mtime → stored-block gzip（zlib 1.2.13 与 1.3.1 的 deflate 输出不同——RFC 1951 BTYPE=00 无压缩算法）→ 固定 uid/gid=0 与 mode（0644/0755；setuid/setgid/sticky 逐字保留——/tmp 01777 压成 0755 会破坏非 root 报告通道）→ COPY-staged `app/` fixture 文件固定 0644（Windows buildkit 模拟 mode 全可执行 vs Linux 真实）→ config history.created 固定 epoch（缓存层 vs 新构建 wall-clock）。execution/registry 探针统一经 normalize_layout_tar 加载规范化产物；docker load 对 OCI layout tar 报告 manifest digest 为镜像 ID（实测）。发现并修复 docker_executor 的 Linux attach 流缺陷（http.client.SocketIO 无 settimeout/recv_into——readinto/read 回退 + 协议声明）。7 轮 runner 验证（layer_diag raw/norm + 成员明细逐层定位）后三 job 全绿。
- **评审**：SPEC PASS（2 Important 关闭：GitLab demo job 先构建镜像；loopback 探针参数化）→ re-review PASS；质量 PASS（0C/0I；9 Minor——F1 zip 简化/F8 docstring 已修，F4 "冗余 -newermt 子句" 实测反驳（删除改变冻结 digest）回滚，其余记录 T37.1）。
- **既有失败记录**（非 T35.1）：tests/integration/windows/test_named_mutex.py 2 项在干净 main 同样失败；mypy 6 errors in 4 files（测试文件）在干净 main 同样存在。
- **WP36 合入 main（SPEC_PROCESS 91）**：分支 codex/wp36 → main（merge commit 65164d5，2026-08-08——AGENT_LOG 冲突（磁盘事件 vs T36 条目）解决后合并）。里程碑 36 三个 session task 全部完成：
  - **T36.1**（36.A）：closed 非秘密 CI/release/deployment 证据 schema（CIReleaseEvidenceV1/ReleaseEvidenceV1/DeploymentEvidenceV1——extra=forbid、Literal 封闭、40/64-hex 内容寻址、时区限定时间戳）+ 只读 `load_and_verify_release_evidence`（AC-30 三方 digest 相等、跨记录 commit/ci_run 对齐、live 终态+24h 新鲜度门）+ CLI。
  - **T36.2**（36.B）：纯零 I/O `verify_release_publication_result`（FrozenReleaseInputsV1/ObservedReleaseResultV1 冻结模型、7 个封闭错误码、确定性优先级 source→tag→wheel→manifest-vs-GHCR→pulled→install→smoke、evidence_write_allowed 仅 ACCEPTED）。
  - **T36.3**（36.C）：committed `render.yaml`（Task 34.B Demo recipe、PORT=8000、/healthz、SOURCE_COMMIT 槽、无 disk/secret/凭据）+ 手写 stdlib 解析器/序列化器（PyYAML 刻意不声明——避免冻结 gate/dependency-closure 级联；SPEC review C1/C2 经 Path B 关闭）+ 20 行矩阵。
- **评审**（每任务两阶段，fresh subagent）：SPEC PASS（T36.1 修 1 Important+3 Minor；T36.2 修 2 Minor pinning；T36.3 修 C1/C2 Critical（Path B）+ 3 Minor）→ re-review PASS；Quality PASS（T36.1 修 6 Minor；T36.2 修 1 Important（mypy 门禁）+ 2 Minor；T36.3 修 6 Minor + 3 残留）→ re-review PASS。4.6/4.7 落实：T36.3 由 subagent 实现（commit 标注）、T36.1/T36.2 driver-inline（如实标注）、worktree/PR/TDD/两阶段评审遵循。
...
## 94. Pre-T37 package-smoke 依赖闭包补正（2026-08-11）

- **现场观测：** 在 `origin/main@816f6da` 派生的 pre-T37 隔离 worktree 中真实运行 `package_smoke` profile，结果为 `1 failed, 8 errors`；共同首因是 formal Python 无法导入 package-smoke 的调用工具：`No module named pipx`。这不是 T33.1 wheel/CLI/WebUI/recovery 行为断言的产品失败，而是测试尚未进入那些断言前的 formal 工具闭包失败。
- **规则判定：** PLAN 原第 43 行（Global Constraints 的 post-T04.1 fail-closed amendment rule）要求发现缺失包时停止当前任务；任何依赖闭包变更必须更新任务合同与验证记录，禁止 ad-hoc install、静默 re-lock 或把宿主机偶然存在的工具当作正式证据。因此拒绝临时 `pip install pipx`，也没有在旧 `.venv-formal` 中补装包后宣称通过。
- **修复：** implementation commit `0d14aba28bde3e5b23f0ac165e346a08bffcf8ed` 将 `pipx==1.16.6` 加入 development direct declaration、hash-complete `requirements/dev.lock`、`DEPENDENCY_CLOSURE_V1` record 与 bootstrap identity probe；新增解析闭包 `argcomplete==3.7.2`、`filelock==3.32.2`、`platformdirs==4.11.2`、`userpath==1.9.2`，复用既有 `click==8.4.2`、`colorama==0.4.6`、`packaging==26.3`。当前闭包为 51 lock entries / 15 direct dependencies；formal identity 为 Python 3.12.4 / pytest 8.4.2 / Ruff 0.16.1 / mypy 2.3.0 / pipx 1.16.6。
...
## 95. Pre-T37 专项门禁真实阻断闭合（2026-08-11）
